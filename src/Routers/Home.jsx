...
export default function Home() {
  ...
  return (
    <div>
      <h2>Home page</h2>
      <button onClick={() => navigate(`/dashboard/${name}`)}>
        Go To Dashboard Using Path Params
      </button>
      ...
      <button onClick={() => navigate("/parenttochild")}>
        ParentToChild ChildToParent
      </button>
      <button onClick={() => navigate("/jesttestinglibrary")}>
        JestTestLogin
      </button>
      <button onClick={() => navigate("/errorBoundary")}>ErrorBoundary</button>
      <button onClick={() => navigate("/interceptor")}>Interceptor</button>
      <button onClick={() => navigate("/throttle_debounce")}>
        Throttle_debounce
      </button>
      <button onClick={() => navigate("/formValidation")}>
        Form Validation
      </button>
    </div>
  );
