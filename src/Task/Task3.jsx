...
export default function Task3() {
  ...
  const addUser = () => {
    const data = [
      ...users,
      { id: users.length + 1, name: newUser, isCompleted: false },
    ];
    console.log(data);
    setUsers(data);
    setNewUser("");
  ...
  const handleData = (event) => {
    ...
    const relatedBooks = list.filter(
      (list) => list.userId == event.target.value,
    );
  ...
  return (
    <div>
      ...
      <div>
        ...
        <ol>
          {relatedUserBooks &&
            relatedUserBooks.map((list) => <li>{list.name}</li>)}
        </ol>
      </div>
      <div
        style={{
